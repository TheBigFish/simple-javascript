...
async function auto_charge() {
    try {
        await navigation_to('charge').then(() =>
            console.log('navigation to charge finish')
        );
        await navigation_to('dock').then(() =>
            console.log('navigation to dock finish')
        );
    } catch (err) {
        console.log(err);
    }
